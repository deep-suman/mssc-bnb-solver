...
566ea02  Validation: Glass identification within +-0.13% of paper Table 9; add handoff doc
cc428f6  Validation: Iris dataset results within ±0.4% of paper Table 8 optimal values
dae97fd  Complete 6-phase MSSC implementation with B&B fix (remove premature CG prune)
```
...
- **Performance fix applied**: `_solve_01qp_exhaustive` vectorized with NumPy;
  threshold lowered from m≤20 to m≤18. Speedup ~100–1000× for m≥15.
...
**Fix**: Removed `if lp_obj >= ub - 1e-6: break` from the TOP of the loop.  
The CG loop now always calls the auxiliary problem; terminates only when `rc >= -1e-6`.

### Bug 2 — Accidental sed side effect
Same fix accidentally removed the pruning check in the main B&B loop (~line 258).  
Restored manually:
```python
if lp_obj >= ub - 1e-6:
    continue  # prune node
...
### Bug 3 — Gurobi dual sign convention
...
File: `glass.data`. Load: `usecols=range(1,10)` (skip ID and class).
...
### Grötschel 202-city (n=202, s=2) — Table 3 — VALIDATED ✅
File: `gr202.tsp` (TSPLIB GEO format — use raw DDD.MM values as 2D Euclidean).
...
### Grötschel 666-city (n=666, s=2) — Table 4 — VALIDATED ✅
...
### Body Measurements (n=507, s=5) — Table 10 — COLUMNS UNCERTAIN ⚠️
File: `body.dat.txt` (25 cols; paper does not specify which 5 were used).  
Best-match columns found: **9, 10, 11, 22, 23** (shoulder girth, chest girth,
waist girth, weight, height). Produces systematic +3–5% above paper.
Solver is correct (1 node, optimal status); gap is column selection, not algorithm.
...
# Cluster — all three args required; indices must be a tuple
cl = Cluster(indices=tuple(...), centroid=np.ndarray, cost=float)

# RMP.solve() returns exactly 3 values
...
res.optimal_cost   # float
res.lp_bound       # float
res.gap_pct        # float
res.n_nodes        # int
res.n_columns      # int
res.time_sec       # float
res.status         # 'optimal' or 'max_nodes'
```
...
- **Branching rarely fires** — Gurobi barrier+crossover returns integer LP vertex
  solutions for set-partitioning problems; this is correct, not a bug
